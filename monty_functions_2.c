 ...
 * @line_number: The current working line number of a Monty bytecodes file.
 *
 * Description: The function computes the modulus of the second value from the
 *              top of the stack by the top value. The result is stored in the
 *              second value node, and the top value is removed. If the top value
 *              is 0, an error is set.
 */
void monty_mod(stack_t **stack, unsigned int line_number)
{
	if ((*stack)->next == NULL || (*stack)->next->next == NULL)
	{
		set_op_tok_error_code(short_stack_error(line_number, "mod"));
		return;
	}
